...
(session-tagged commit, `AGENTS-LOG.md` append, meadow-postgres agent_state store row flip
via `agent_state.py flip` — no state file is ever written — `eval_session` run).
